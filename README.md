...
1. **`claude-chat.js`** - Interactive Chat CLI Application
   - Complete chat interface with Claude
   - `/auth` command for OAuth authentication
   - `/model` command to switch between models
   - `/clear`, `/help`, `/exit` commands
   - Maintains conversation history

2. **`claude-sdk-final.js`** - Complete SDK Implementation
   - Auto-detects authentication methods (OpenCode or API key)
   - Seamless backend switching
   - Production-ready for integration
   - Handles both OAuth tokens (via OpenCode) and API keys

3. **`commentary.md`** - Technical Analysis & Findings
   - Documents OAuth token restrictions
   - Explains the MIT license paradox with OpenCode
   - Details our investigation and discoveries
   - Critical reading for understanding limitations

4. **`example.js`** - Usage Examples
   - Shows how to use the SDK
   - Demonstrates OpenCode backend method
   - Demonstrates API key method
...
6. **`package-lock.json`** - Dependency Lock File
   ...
   - Guarantees everyone gets same package versions

7. **`node_modules/`** - Dependencies Directory (not in git)
   - Contains all installed npm packages
   - Created when you run `npm install`
   - Should be in `.gitignore`
   - Recreated from package-lock.json
